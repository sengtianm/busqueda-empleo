...
| FichaFuente | ✅ existe (como `FichaFuente`; **no existe `FichaAcceso`**) | 135 |
...
⚠️ Detalle factual menor: `set_indice INTEGER DEFAULT ''` en el esquema de `ofertas` (línea 92) — mezcla de tipo SQL (`INTEGER`) con valor por defecto de texto. Funciona en SQLite, pero es incoherente tipográficamente con el 13A.
...
| Ítem | Estado | Línea |
|---|---|---|
| `should_retry(codigo_motivo)` | ✅ | 62 (códigos en 25-30: `fuente_inalcanzable`, `timeout_ingreso`, `timeout_consulta`, `timeout_captura`) |
...
- ⚠️ Nota de nomenclatura: el plan (§6.3) los llamaba `example_fuente`/`example_set`/`example_politicas`; la implementación los nombró `example_ficha_fuente`/`example_set_filtros`/`example_politicas_captura` — equivalentes funcionales
...
- Existe (380 líneas). Clases: `FlowError` (66) y `LinkedInAdapter` (75)
...
- **INT-001/INT-003 (DOC-12):** ✅ implementa — DOC-12 define INT-001 "Job search platform" (769) e INT-003 "Automated browser" (793); el adaptador implementa el patrón con navegación sobre objeto `page` inyectado y cuatro métodos que cubren el contrato. Clasifica fallos con `codigo_motivo` oficial (DOC-06 §11) y declara eventos con tipología `EventoAlmacen` (311-320)

## 10. Estado de la documentación (criterios del Plan de Preparación)

| Doc | Criterio | Estado | Evidencia |
|---|---|---|---|
| Document 13A | Entidades Corrida/Sesión/Bloqueo/Evento formalizadas | ✅ | Inventario (24-26); Corrida §2.14 (1326), Sesión §2.15 (1402), Bloqueo §2.16 (1470), Evento (953); versión 1.3 (2516) |
| DOC-13 | Ídem | ⚠️ | Solo decisiones D1/D2/D3 en §13.8 (1276-1283); las entidades formalizadas viven en el 13A, no en DOC-13 (documento de política) |
...
| DOC-06 | Catálogo ERR- + mapeo ER-* + Grupo A/B + reintentos condicionales | ✅ | "Module 1: Error Catalog" (3308-3390): por nodo (3312), tabla oficial con mapeo + retry (3332), política condicional (3365), Grupo A/B (3379), estados de terminación (3390) |
| DOC-01 | RF del M1 + nota `estado='discovered'` | ✅ | §20 (1437): RF-M1-001..009 (1445-1476); nota C5 (1481); nota D1 (1480-1487); conjunto imprescindible §21 (1489-1505) |
| DOC-12 | CMP-001 desglosado en submódulos | ✅ | Estructura interna (280-298): `run_context.py`, 13 nodos, `adaptadores`; SRV-003/004/005; INT-001 (769), INT-003 (779); almacén de credenciales (298) |
| DOC-09 + Anexo 9A | Criterio verificable de ingreso LinkedIn | ✅ | DOC-09 §6 (301-302: `global-nav` + `timeout_ingreso`); §6.4 captcha (337-356); 9A DE-LI-010 (295-320: criterio DOM verificable, prohibición de reintento tras captcha) |

Complementarios verificados: Apéndice 5A con prefijos COR/SES/EVT/BLO (128-131) ✅ · DOC-00 glosario (362-427) ✅
...
**Listo para la Sub-fase 4.1 (nodo INICIO):** la infraestructura de preparación está completa y validada (Ruff 0, mypy 0, 102 tests). Todos los ítems de checklist §6 del Plan de Preparación están cumplidos salvo la construcción misma de los nodos (`modules/discovery/nodes/` está vacío).