...
| 4.17 | Duplication unifications from the P3 improvement plan (Lote C; decision D24) | Ficha técnica, decision log | ✅ | (1) `ahora()` + `FORMATO_TIMESTAMP` + `TIPOS_ACCESO` as public helpers in `shared/utilidades.py`; 7 call sites updated (persistence, run_context, inicio, finalizar, control_fuentes, captura, registro); local `ahora` vars renamed to `marca` in persistence (shadowing). (2) `escribir_evento_seguro(datos, contexto_log="")` in `shared/persistence.py` replaces the 5 per-node try/except wrappers (inicio, captura, control_fuentes, registro, finalizar) — each node keeps its exact event dict; RN-04 preserved (write failure logs, never aborts). (3) `_enviar(url, headers, payload, timeout, servicio)` in `shared/ia_service.py`; `_send_local`/`_send_cloud` remain public thin wrappers with `@retry_decorator()`; LLM-001..003 unchanged. (4) `_construir_politicas` merges `_politicas_desde_global` via `{**config_captura, **por_fuente}` (defaults 5/25/10/10/`pausa_aleatoria` identical). (5) `TIPOS_ACCESO` unique constant imported by INICIO and RunContext. (6) Adapter `_revisar_estado` unifies `_revisar_estado_pagina`/`_revisar_estado_captura`; `busqueda.py` builds failure `SearchResult` via `_resultado_fallo`. Discarded: full structural source validation INICIO↔RunContext and `_es_obsoleto` mirror (unchanged). Documented: cosmetic message changes (capture empty-page evidence, LLM-003 local capitalization, unified failure log) and F-003 pre-existing drift (termination event written in a single attempt, only `actualizar_corrida` retries; ficha wording kept as known drift). Docs: decision log v1.10 (D24), ficha as-built note, AGENTS.md test count 302 + `shared/utilidades.py` in Spanish exception list. 302 tests passing; ruff/mypy clean. |
