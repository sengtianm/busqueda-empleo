...
**Attributes:** `id`, `fuente_id`, `company_id`, `location_id`, `id_corrida`, `id_sesion`, `indice_set`, `id_externo`, `fecha_ultima_verificacion`, `enlace`, `title`, `original_description`, `publication_date`, `fecha_descubrimiento`, `status`, `active`, `observations`, `fecha_creacion`, `update_date`, `ubicacion_nombre`, `modalidad`, `empresa`, `id_duplicidad`. **As-built D41 (2026-08-25):** physical column `empresa TEXT DEFAULT 'N/R'` added — raw company text from the listing card, written by Module 1 at registration (mirrors `ubicacion`; never overwritten on re-registration; historical rows keep `'N/R'`).
