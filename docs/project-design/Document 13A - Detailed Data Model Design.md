...
**Attributes:** `id`, `name`, `nombre_normalizado`, `website`, `linkedin`, `sector`, `size`, `description`, `headquarters`, `type`, `founded`, `specialties`, `observations`, `fecha_creacion`, `update_date`. (**As-built D39, 2026-08-25:** the four enrichment extras are physical columns `sede`/`tipo`/`fundacion`/`especialidades`; all eight enrichment fields default `'N/R'`.)
...
**Observations:** Represents only the organization offering the vacancy. The same company may publish multiple offers over time and through different sources while maintaining a single record. **As-built D33 (2026-08-20):** Module 2 (Preparation — Nodo 2) populates this catalog via upsert by `nombre_normalizado` (no AI); the deep enrichment of `sitio_web`, `sector`, `tamano` and `descripcion` is decoupled from the critical path (decision D4) and runs as step 6 of Finalizar Proceso, controlled by `preparacion.profundidad_catalogo_empresa`. **As-built D39 (2026-08-25):** enrichment moved into Nodo 2 itself (step 2): right after each company is registered, its public LinkedIn profile is visited as a guest and eight fields are completed (`website`, `sector`, `size`, `description` + `headquarters`, `type`, `founded`, `specialties`), `'N/R'` when the page does not show them; autocuración re-visits still-all-'N/R' companies on later runs; no per-run cap (`profundidad_catalogo_empresa` redefined as an optional brake, 0 = unlimited); traceability log-only; the Finalizar step 6 (H2 stub) was removed.
...
- `website` — Official website. Text; optional. Public; Permanent. Actual name: `sitio_web`. **As-built D39 (2026-08-25):** populated by the step-2 enrichment (Nodo 2, guest profile visit); `'N/R'` when unreported.
...
- `sector` — Economic sector. Catalog; optional; FK Catalog. Domain: Business Sectors. Constraint: valid catalog value. Internal; Permanent. **As-built D39 (2026-08-25):** free text read from the public profile by the step-2 enrichment; `'N/R'` when unreported (catalog FK not implemented in MVP).
- `size` — Company size classification. Catalog; optional; FK Catalog. Domain: Company Size. Constraint: valid catalog value. Internal; Permanent. Actual name: `tamano`. **As-built D39 (2026-08-25):** free text (employee range) from the public profile; `'N/R'` when unreported.
- `description` — General company description. Long Text; optional. Public; Permanent. Actual name: `descripcion`. **As-built D39 (2026-08-25):** clean embedded profile description (share-card meta fallback), whitespace-collapsed and capped at 2000 chars; `'N/R'` when unreported.
- `headquarters` — Main office location. Text; optional. Public; Permanent. Actual name: `sede`. **As-built D39 (2026-08-25):** added with the step-2 enrichment; `'N/R'` when unreported.
- `type` — Organization type (e.g. privately held). Text; optional. Public; Permanent. Actual name: `tipo`. **As-built D39 (2026-08-25):** added with the step-2 enrichment; `'N/R'` when unreported.
- `founded` — Founding year. Text; optional. Public; Permanent. Actual name: `fundacion`. **As-built D39 (2026-08-25):** added with the step-2 enrichment; `'N/R'` when unreported.
- `specialties` — Areas of specialty. Text; optional. Public; Permanent. Actual name: `especialidades`. **As-built D39 (2026-08-25):** added with the step-2 enrichment; `'N/R'` when unreported.
- `observations` — Additional relevant information. Long Text; optional. Internal; Permanent.
