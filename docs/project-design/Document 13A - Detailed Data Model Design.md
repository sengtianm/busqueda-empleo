...
| Corrida | Operational | Traceability | Execution instance of the Discovery module (run); every record generated by the module (errors, success events, sessions, offers) is anchored to its `run_id` (RN-01). |
| Sesion | Operational | Traceability | Audit of each platform session used by the Discovery module during a run. |
| Bloqueo | Operational | Concurrency | Persistent lock that guarantees a single active run of the Discovery module. |

> **Scope of implemented persistence (decisions 2026-07-30 and 2026-08-07):** the MVP database (`job_search.db`) persists a trimmed subset of the model above: `secuencia_ids`, `fuentes`, `empresas`, `ubicaciones`, and `ofertas`. The Discovery module (Module 1, Phase 4) additionally defines its own logical stores — `eventos`, `sesiones`, `corridas`, and `bloqueo` — as tables of the same single SQLite file (decision D2, 2026-08-07). The remaining entities of this inventory are deferred to later modules and are defined in their corresponding phases. This note formalizes the actual persistence scope; the full inventory above remains the target model.
...
- The Discovery module (Module 1) records every newly discovered offer with status `discovered` (decision C5, 2026-08-07); the transition towards other states belongs to the Processing module (Module 2).
...
- `active` is a catalog attribute intended for external administration; the Discovery module does not filter sources by it at runtime (decision D1, 2026-08-07).
...
| run_id | UUID | Yes | Identifier of the run (Corrida) that generated the event (module 1 traceability, RN-01). |
| source_id | UUID | No | Identifier of the source on which the event occurred (optional, module 1). |
| session_id | UUID | No | Identifier of the platform session in which the event occurred (optional, module 1). |
| set_indice | Integer | No | Index of the filter set in which the event occurred (optional, module 1). |
| offer_id | UUID | No | Reference to the offer related to the event, when applicable. |
| tipo | Catalog | Yes | Classification of the event: `error` or `suceso` (module 1 typology). |
| codigo | Text | No | Business code of the event (e.g., `ERR-05`, `EVT-01`) per the Discovery module technical sheet. |
| evidencia | Long Text | No | Evidence of the event (screenshots, traces, or raw snippets; never credentials or session tokens). |
| event_type | Catalog | Yes | Type of event recorded. |
| affected_entity | Text | Yes | Name of the entity on which the event occurred. |
| entity_id | UUID | Yes | Identifier of the record affected by the event (for run-level events, the `run_id`). |
...
- Every event must be associated with a run (`run_id`) in module 1; `offer_id` is required only when the event belongs to a specific offer.
- Every event must record the exact moment when it occurred.
- Every module 1 event must record its `tipo` (`error` or `suceso`) and, when applicable, its `codigo` from the Discovery module technical sheet.
...
In the Discovery module (module 1), every event additionally records its origin run (`run_id`), its typology `tipo` (`error` or `suceso`, decision 2026-08-07), and, when applicable, its business `codigo` (see the Discovery module technical sheet — business codes `ERR-nn` and `EVT-nn`).
...
The **Corrida** entity represents each execution instance (run) of the Discovery module (module 1).

A run is created at startup, acquires the module's persistent lock, processes its sources, and ends with a termination motive; every record generated by the module is anchored to its `run_id`.

---

### Purpose

Provide a single identification anchor for each execution, enabling full traceability of all records of the module and supporting the recovery of concurrent or interrupted runs.
...
| Attribute | Logical Type | Required | Description |
|----------|-------------|-------------|-------------|
| id | UUID | Yes | Unique identifier of the run record. |
| run_id | UUID | Yes | Public unique identifier of the run (RN-01); referenced by every record of the module (offers, events, sessions). |
| start_timestamp | Date/Time | Yes | Date and time when the run started. |
| end_timestamp | Date/Time | No | Date and time when the run ended normally or was terminated (concurrency, error, or no sources). |
| estado | Catalog | Yes | Status of the run: `en_ejecucion`, `corrida_completada`, `sin_fuentes`, `error`, `concurrencia`. |
...
| Related entity | Relationship | Cardinality |
|----------------------|----------|--------------|
| Offer | Captures under | 1 : N |
| Event | Anchors | 1 : N |
| Sesion | Registers | 1 : N |
| Bloqueo | Is protected by | 1 : 0..1 |

---

### Constraints

- `run_id` must be unique within the system.
- Each record generated by the module (errors, success events, sessions, offers) must reference its `run_id`.

---

### State machine

Run statuses and their transitions are defined by the technical sheet of the Discovery module (INICIO and FINALIZAR PROCESO nodes).

---

### Observations

- Physical table: `corridas` (decision D2, 2026-08-07).
...
The **Sesion** entity represents the audit record of a platform session successfully established by the Discovery module (module 1).

It records the essential session data required for auditing; it does not store credentials, tokens, or cookies.

### Purpose

Provide a minimal viable session audit (`session_id`, `run_id`, `source_id`, `set_indice`, `timestamp`, `total_declarado`, `conteo`, `estado`) per source and run (decision D3, 2026-08-07; RN-07 of the technical sheet).
...
| Attribute | Logical Type | Required | Description |
| --------- |-------------|----------|-------------|
| id | UUID | Yes | Unique identifier of the session record. |
| session_id | UUID | No | Unique platform session identifier assigned by the source. |
| run_id | UUID | Yes | Run under which the session was established. |
| source_id | UUID | Yes | Source on which the session was established. |
| set_indice | Integer | No | Index of the filter set to which the session belongs. |
| timestamp | Date/Time | Yes | Date and time when the session was established. |
| total_declarado | Integer | No | Total number of offers declared by the source, when exposed. |
| conteo | Integer | Yes | Number of offers captured during the session. |
| estado | Catalog | Yes | Final state of the session (e.g., `activa`, `cerrada`, `expirada`). |
...
- (session_id, set_indice) per source
...
- A session record is created only after a successful entry (decision D3); failed attempts are reported as events, not as sessions.
- Sessions record the essential audit fields only.
...
The technical sheet mentions additional audit fields (e.g., `conteo_primera_pagina`, `hay_mas_paginas`, `coherencia`, `estado_auditoria`); the MVP limits itself to the essential fields of decision D3.
...
The **Bloqueo** entity implements the persistent concurrency lock of the Discovery module (module 1): at most one active run at a time.

### Purpose

Guarantee a single active run per module and allow the recovery of runs interrupted by obsolescence (old run lock), as defined in the technical sheet (ERR-06/07/09).
...
| Attribute | Logical Type | Required | Description |
| ----------|-------------|----------|-------------|
| id | UUID | Yes | Unique identifier of the lock record. |
| run_id | UUID | Yes | Identifier of the run that currently owns the lock. |
| timestamp | Date/Time | Yes | Date and time when the lock was acquired. |
| umbral_obsolescencia | Integer | Yes | Time limit (seconds) after which the lock may be considered obsolete and taken over by a new run. |
| estado | Catalog | Yes | State of the lock: `activo`, `obsoleto`, `liberado`. |
...
- If the `timestamp` of an active lock is older than `umbral_obsolescencia`, a new run may take over the lock (obsolete lock).
...
- Configuration of `umbral_obsolescencia` is centralized in the system configuration file (no hardcoded values).
...
> Each event in the Discovery module (module 1) is additionally classified by the `tipo` attribute of the **Event** entity: `error` or `suceso` (events with any of the values above; decision 2026-08-07).
...
- A **Corrida** may capture many **Offers** (they are anchored to its `run_id`).
- A **Corrida** may anchor many **Events**.
- A **Corrida** may register many **Sesiones**.
- A **Corrida** may be protected by a **Bloqueo** (at most one active lock).
...
During the Discovery stage (module 1), each **Corrida** (run) anchors every record it generates — **Offers** (`run_id`), **Events**, and **Sesiones** — and is protected by a **Bloqueo** while active.
...
- Every record of the Discovery module (offers, events, sessions) can be traced back to its run through `run_id` (RN-01).
...
| Offer | run_id | Run that discovered the offer. | UUID | No | | | Corrida | | | Traceability of the discovery module (RN-01). | Internal | Permanent | Actual name: `run_id`. |
| Offer | session_id | Platform session used to discover the offer. | UUID | No | | | | | | Optional, module 1. | Internal | Permanent | Actual name: `session_id`. |
| Offer | set_indice | Index of the filter set that produced the offer. | Integer | No | | | | | | Optional, module 1. | Internal | Permanent | Actual name: `set_indice`. |
| Offer | id_externo_url | External identifier of the offer in the source of origin. | Text | No | | | | | | Alias of `source_identifier`; best effort. | Public | Permanent | Actual name: `id_externo_url`. |
...
| Source | active | Indicates whether the source is enabled for offer discovery. | Boolean | Yes | | | | `true` | | Catalog attribute for external administration; the Discovery module does not filter by it at runtime (decision D1, 2026-08-07). | Internal | Permanent | |
...
| Event | run_id | Run that generated the event. | UUID | Yes | | | Corrida | | | Mandatory (module 1, RN-01). | Internal | Permanent | Actual name: `run_id`. |
| Event | source_id | Source on which the event occurred. | UUID | No | | | Source | | | Optional, module 1 traceability. | Internal | Permanent | |
| Event | session_id | Platform session in which the event occurred. | UUID | No | | | | | | Optional, module 1. | Internal | Permanent | |
| Event | set_indice | Index of the filter set in which the event occurred. | Integer | No | | | | | | Optional, module 1. | Internal | Permanent | |
| Event | offer_id | Reference to the offer related to the event. | UUID | No | | | Offer | | | Required only when the event belongs to a specific offer. | Internal | Permanent | |
| Event | tipo | Classification of the event: `error` or `suceso`. | Catalog | Yes | | | Catalog | | `error`, `suceso` | Module 1 typology (decision 2026-08-07). | Internal | Permanent | |
| Event | codigo | Business code of the event (e.g., `ERR-01`, `EVT-01`). | Text | No | | | | | | Per the Discovery module technical sheet (business codes). | Internal | Permanent | |
| Event | evidencia | Evidence of the event (screenshots, traces). | Long Text | No | | | | | | Never credentials or session tokens. | Internal | Permanent | Actual name: `evidencia`. |
...
| Entity | Attribute | Description | Type | Req. | PK | AK | FK | Default | Domain | Constraints | Sensitivity | Persistence | Observations |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| Corrida | id | Unique identifier of the run record. | UUID | Yes | Yes | | | | | | Internal | Permanent | |
| Corrida | run_id | Public unique identifier of the run. | UUID | Yes | | Yes | | | | Referenced by every record of the module (RN-01). | Internal | Permanent | Actual name: `run_id`. |
| Corrida | start_timestamp | Date and time when the run started. | Date/Time | Yes | | | | | | | Internal | Permanent | |
| Corrida | end_timestamp | Date and time when the run ended. | Date/Time | No | | | | | | Normal, concurrency, or failure termination. | Internal | Permanent | |
| Corrida | estado | Status of the run. | Catalog | Yes | | | Catalog | `en_ejecucion` | `en_ejecucion`, `corrida_completada`, `sin_fuentes`, `error`, `concurrencia` | Follows the technical sheet of the Discovery module. | Internal | Permanent | Actual name: `estado`. |
...
| Entity | Attribute | Description | Type | Req. | PK | AK | FK | Default | Domain | Constraints | Sensitivity | Persistence | Observations |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| Sesion | id | Unique identifier of the session record. | UUID | Yes | Yes | | | | | | Internal | Permanent | |
| Sesion | session_id | Platform session identifier assigned by the source. | UUID | No | | Yes | | | | Unique per source. | Internal | Permanent | |
| Sesion | run_id | Run under which the session was established. | UUID | Yes | | | Corrida | | | | Internal | Permanent | |
| Sesion | source_id | Source on which the session was established. | UUID | Yes | | | Source | | | | Internal | Permanent | |
| Sesion | set_indice | Index of the filter set to which the session belongs. | Integer | No | | Yes | | | | Alternate key with session_id. | Internal | Permanent | |
| Sesion | timestamp | Date and time when the session was established. | Date/Time | Yes | | | | | | | Internal | Permanent | |
| Sesion | total_declarado | Total of offers declared by the source, when exposed. | Integer | No | | | | | | | Internal | Permanent | |
| Sesion | conteo | Number of offers captured during the session. | Integer | Yes | | | | | | | Internal | Permanent | |
| Sesion | estado | Final state of the session. | Catalog | Yes | | | Catalog | | `activa`, `cerrada`, `expirada` | | Internal | Permanent | |
...
| Entity | Attribute | Description | Type | Req. | PK | AK | FK | Default | Domain | Constraints | Sensitivity | Persistence | Observations |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| Bloqueo | id | Unique identifier of the lock record. | UUID | Yes | Yes | | | | | | Internal | Permanent | |
| Bloqueo | run_id | Run that owns the lock. | UUID | Yes | | | Corrida | | | | Internal | Permanent | |
| Bloqueo | timestamp | Date and time when the lock was acquired. | Date/Time | Yes | | | | | | | Internal | Permanent | |
| Bloqueo | umbral_obsolescencia | Time limit (seconds) after which the lock becomes obsolete. | Integer | Yes | | | | | From configuration. | No hardcoded values; defined in the configuration file. | Internal | Permanent | |
| Bloqueo | estado | State of the lock. | Catalog | Yes | | | Catalog | `activo` | `activo`, `obsoleto`, `liberado` | Single active lock per run. | Internal | Permanent | |
