...
## 16.1. Event and session audit formats for Module 1 (Opportunity Discovery)

These conventions establish the official formats for recording events and audit sessions generated by the Opportunity Discovery module (Module 1) of the automation.

### Formats for events

Every event generated by the Discovery module shall record, at minimum, the following elements, aligned with the Event entity of the official data model (DOC-13 / 13A):

| Attribute | Description |
|-----------|-------------|
| run_id | Identifier of the run (Corrida) that generated the event (RN-01). |
| source_id | Identifier of the source on which the event occurred (optional). |
| session_id | Identifier of the platform session in which the event occurred (optional). |
| set_indice | Index of the filter set in which the event occurred (optional). |
| timestamp | Date and time when the event occurred. |
| tipo | Event classification: `error` or `suceso`. |
| codigo | Business code of the event (convention `ERR-nn` / `EVT-nn`, see DOC-06, Section 11). |
| evidencia | Evidence of the event (screenshots, traces, or raw snippets; never credentials or session tokens). |

The following example shall be used as reference and complies with the official event template (Appendix 5C, C.11):

```markdown
Date and time: 2026-08-07 10:23:45 (local)

Module: Discovery (M1)

Process: Corrida 3f2a-11 (set_indice 0)

Event: EVT-01 (oferta_no_capturada)

Level: suceso

Result: Éxito - lote procesado, una oferta no capturada

Observations: total_declarado=37, conteo=25, paginas_consumidas=1; evidencia adjunta (registro de la oferta)
```

### Session audit format

The audit of each platform session shall record, exclusively, the minimal viable fields defined by decision D3 and the Sesion entity of the official data model (DOC-13 / 13A):

`session_id`, `run_id`, `source_id`, `set_indice`, `timestamp`, `total_declarado`, `conteo`, `estado`.

Session records are created only after a successful entry; failed attempts are reported as events, never as sessions.

### Code convention per node

Business codes shall follow the convention `ERR-nn` (errors) and `EVT-nn` (events) established in DOC-06 (Section 11, Error Handling by Module), preserving the full catalog defined for the Discovery module and its mapping to official error categories.
