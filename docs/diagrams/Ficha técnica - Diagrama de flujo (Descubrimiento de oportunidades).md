...
- **Trazabilidad**: todo registro incluye `run_id`; cuando aplica, también `source_id`, `session_id` y `set_indice`.
- **Registro**: en `"errores o sucesos"` si está disponible; si no, registro crítico local (consola/archivo documentado). Todo evento incluye `run_id`, timestamp, tipo y descripción; los descartes/fallos por fuente incluyen `source_id`.
...
1. Crea instancia de corrida con `run_id` único y timestamp.
...
- **Técnica**: `run_id`, conexiones, bloqueo de concurrencia, estado inicial.
...
### Salidas
- Contexto inicializado: `run_id`, timestamp de inicio, lista filtrada de fuentes válidas —cada una con acceso, `sets_de_filtros` y políticas efectivas—, iterador en “ninguna fuente seleccionada aún”, contadores/variables, conexiones activas, bloqueo adquirido.
...
### Reglas de negocio
- **RN-01**: toda corrida tiene `run_id` único; todo registro del módulo queda enlazado a él.
...
- **RN-07**: identificadores de fuente únicos por configuración; validados aquí y sostienen la trazabilidad por `source_id` de nodos posteriores.
...
- **VAL-04**: bloqueo legible; obsolescencia por timestamp contra umbral configurable; timestamps coherentes.
...
| ERR-12 | Ficha por fuente incompleta/inconsistente | 5 | Evento crítico con `run_id` + `source_id` | Descartar fuente y continuar | continúa |
...
- **Impactos aprobados**:
  - `run_id` se registra en BD del módulo.
  - `"Finalizar Proceso"` libera el bloqueo.
  - RN-03 formalizada en `"¿Quedan fuentes por procesar…?"` v1.0.
  - RN-07 sostiene trazabilidad por `source_id` en `"Seleccionar la siguiente fuente pendiente"`.
...
- Bloqueo persistente con `run_id` y timestamp; umbral de obsolescencia configurable.
...
### Pasos funcionales
1. **Recibir disparador e instanciar corrida**. Entrada: evento manual/programado. Proceso: crear instancia, generar `run_id`, timestamp. Salida: instancia. Val: VAL-01. Err: ERR-01.
...
4. **Validar concurrencia**. Entrada: `run_id`, estado de bloqueo, umbral. Proceso: leer bloqueo; activo no obsoleto → terminación controlada; obsoleto → sobrescribir y continuar; inexistente → marcar bloqueo. Salida: bloqueo adquirido o terminación. Val: VAL-04. Err: ERR-06, ERR-07, ERR-08, ERR-09.
...
6. **Inicializar estado**. Entrada: `run_id`, lista filtrada. Proceso: fijar iterador en “ninguna fuente seleccionada aún”; inicializar contadores. Salida: contexto completo. Val: iterador/contadores iniciales. Err: ERR-10.
...
### Entradas
Contexto de INICIO: lista de fuentes, `run_id`, conexiones activas, bloqueo adquirido.
...
- **VAL-04**: en rama No, `sin_fuentes` fijado con `run_id` y timestamp antes de entregar control.
...
4. **Rama No**. Entrada: resultado No, `run_id`. Proceso: fijar `sin_fuentes` y entregar control. Salida: flujo a `"Finalizar Proceso"`. Val: VAL-04. Err: ninguna.
...
### Entradas
Contexto: lista de fuentes no vacía, iterador, `run_id`.
...
- **VAL-04**: en No, `corrida_completada` fijado con `run_id` y timestamp.
...
4. **Rama No**. Entrada: resultado No, `run_id`. Proceso: fijar `corrida_completada` y entregar control. Salida: flujo a `"Finalizar Proceso"`. Val: VAL-04. Err: ninguna.
...
### Descripción funcional
Identifica la primera entrada no procesada en orden de configuración, la marca como procesada en el momento de selección, la fija como fuente corriente con `source_id`, posición, acceso y filtros básicos, y entrega control.

### Entradas
Contexto con contrato Sí: lista, iterador, `run_id`, existencia de fuente pendiente.
...
- **RN-06**: `source_id` + `run_id` deben incluirse en todo registro posterior.
...
- **VAL-03**: tras mutación, iterador refleja procesada y fuente corriente tiene `source_id` y parámetros completos.
...
- **Impactos aprobados**:
  ...
  - Registros incorporan `source_id`.

### Notas de implementación
- Mutación atómica: marcar procesada + fijar fuente corriente.
- Iteración por posición; `source_id` solo trazabilidad.
...
2. **Determinar siguiente pendiente**. Entrada: lista, iterador. Proceso: primera entrada no procesada en orden. Salida: posición y `source_id`. Val: VAL-02. Err: ERR-02.
3. **Mutar contexto**. Entrada: posición y `source_id`. Proceso: marcar procesada; fijar fuente corriente con parámetros. Salida: contexto actualizado. Val: VAL-03. Err: ERR-03.
...
Ciclo de intento:
- códigos reintentables: `fuente_inalcanzable`, `timeout_ingreso`;
...
```
En éxito crea `session_id` único y guarda handle + `session_id`. En fallo cierra canal y garantiza ausencia de sesión. Entrega control a `"¿El ingreso fue exitoso?"`.

### Entradas
- Contexto: fuente corriente con ficha completa: `source_id`, URL/plataforma, tipo `publico|con_autenticacion`, referencia de credenciales, criterio de éxito, filtros básicos, timeout; `run_id`.
...
- En éxito: sesión activa (`session_id` + handle).
...
- **RN-03**: reintentos condicionales hasta N configurables (default 2), con backoff, solo para `fuente_inalcanzable` y `timeout_ingreso`. Cada reintento cierra canal previo y reejecuta desde acceso. No reintentables: `credenciales_no_disponibles`, `autenticacion_rechazada`, `bloqueo_plataforma`, `criterio_no_cumplido`, `error_interno_fuente`.
...
- **RN-05**: `session_id` solo en éxito; en fallo canal cerrado y sin sesión.
- **RN-06**: evidencia acotada y sin datos sensibles.
- **RN-07**: eventos incluyen `run_id` y `source_id`.
...
- **VAL-05**: éxito → sesión con `session_id` y handle; fallo → canal cerrado y sin sesión.
...
| ERR-04 | `timeout_ingreso` | Sí | 3/4 | Ídem ERR-03 | fallo de fuente |
...
**Contrato de códigos**: `ingreso_exitoso`, `credenciales_no_disponibles`, `fuente_inalcanzable`, `timeout_ingreso`, `autenticacion_rechazada`, `bloqueo_plataforma`, `criterio_no_cumplido`, `error_interno_fuente`.
...
- **Impactos aprobados**:
  ...
  - `session_id` alimenta `"control de sesiones"` y registros de ofertas;
...
- `session_id` único y vinculado a `run_id` y `source_id`.
...
4. **Autenticar si aplica**. Entrada: canal, credenciales. Proceso: autenticar dentro de timeout; `timeout_ingreso` reintentable → reejecutar desde paso 3; `autenticacion_rechazada`/`bloqueo_plataforma` → fallo; `publico` sin efecto. Salida: estado post-autenticación. Val: VAL-03, VAL-06. Err: ERR-04, ERR-05, ERR-06, ERR-08.
5. **Evaluar criterio y construir `entry_result`**. Entrada: estado del canal, criterio, contador. Proceso: aplicar criterio; construir `entry_result`; éxito → `session_id` + handle; fallo → cerrar canal sin sesión. Salida: `entry_result`; sesión lista o ausente. Val: VAL-04, VAL-05. Err: ERR-07, ERR-08, ERR-09.
...
### Entradas
Contexto: `entry_result`, sesión si éxito, `run_id`, `source_id`.
...
- **VAL-03**: si éxito, sesión presente con handle y `session_id`.
...
| Código | Error / excepción | Detección | Registro | Acción | Estado |
|---|---|---:|---|---|---|
| ERR-01 | `entry_result` ausente o corrupto | 1 | Evento crítico con `run_id` + `source_id` | Aborto | `error` |
| ERR-02 | Estructura inválida o éxito sin sesión | 2 | Evento crítico con `run_id` + `source_id` | Aborto | `error` |
...
### Objetivo
Seleccionar el siguiente set pendiente, ejecutar la búsqueda con ese set dentro de la sesión, interpretar la respuesta y producir `search_result` con primera página de referencias, paginación, metadatos y `set_indice`.
...
- **RN-06**: reintentos solo para `fuente_inalcanzable` y `timeout_consulta`; resto fallo inmediato.
...
- **RN-09**: evidencia acotada; trazabilidad `run_id` + `source_id` + `session_id` + `set_indice`.
...
- **VAL-05**: `search_result` completo con `set_indice` antes de entregar control.
...
| ERR-04 | `timeout_consulta` | Sí | 3 | Ídem ERR-03 | fallo de set |
...
**Contrato de códigos**: `consulta_exitosa`, `filtros_no_aplicables`, `respuesta_invalida`, `fuente_inalcanzable`, `timeout_consulta`, `sesion_expirada`, `error_interno_consulta`.
...
- **Contratos entregados**: `search_result` con `set_indice`; iterador avanzado.
- **Impactos aprobados**:
  ...
  - Registros incorporan `set_indice`.
...
- Iterador vinculado a `source_id`; reiniciar si cambió fuente; marcar al seleccionar.
...
2. **Seleccionar set pendiente**. Entrada: sets, iterador, `source_id`. Proceso: reiniciar si cambió fuente; seleccionar primer no procesado; marcar procesado; fijar `set_corriente`. Salida: `set_corriente`, iterador avanzado. Val: VAL-06. Err: ERR-01.
...
5. **Construir `search_result`**. Entrada: resultados o fallo, `set_indice`, intentos. Proceso: construir estructura completa. Salida: `search_result` en contexto. Val: VAL-05. Err: ERR-07, ERR-08.
...
### Entradas
Contexto: `search_result`, `run_id`, `source_id`, `session_id`.
...
- **VAL-02**: estructura completa incluido `set_indice`; si éxito, lista presente con conteo ≥ 0.
...
| Código | Error / excepción | Detección | Registro | Acción | Estado |
|---|---|---:|---|---|---|
| ERR-01 | `search_result` ausente o corrupto | 1 | Evento crítico con `run_id`, `source_id`, `session_id`, `set_indice` | Aborto | `error` |
| ERR-02 | Estructura inválida o éxito sin lista | 2 | Evento crítico con `run_id`, `source_id`, `session_id`, `set_indice` | Aborto | `error` |
...
- **Impactos aprobados**:
  - registro consume `search_result`, tipifica error/suceso, incluye `set_indice`, enruta a sets;
...
### Descripción funcional
Lee insumos de captura. En primer lote del par `(session_id, set_indice)` escribe auditoría con tolerancia a degradación. Determina alcance del lote:
...
### Entradas
- `search_result`: `ofertas_primera_pagina`, `estado_paginacion`, `set_indice`, `total_declarado`.
- Sesión activa: `session_id` + handle.
...
- **RN-06**: reintentos solo para `fuente_inalcanzable` y `timeout_captura`; resto fallo inmediato. Ante fallo de lote, conservar lote parcial.
- **RN-07**: auditoría por `(session_id, set_indice)` en primer lote:
  ```text
  session_id, run_id, source_id, set_indice, timestamp,
...
| ERR-04 | `timeout_captura` | Sí | 3–4 | Ídem ERR-03 | fallo de lote |
...
**Contrato de códigos**: `captura_exitosa`, `fuente_inalcanzable`, `timeout_captura`, `sesion_expirada`, `bloqueo_plataforma`, `respuesta_invalida`, `error_interno_captura`; evento por oferta: `oferta_no_capturada`.
...
- **Contratos entregados**: `capture_batch`, `estado_captura`, auditoría por `(session_id, set_indice)`.
- **Impactos aprobados**:
...
2. **Auditoría de sesión**. Entrada: `session_id`, `set_indice`, `search_result`. Proceso: si primer lote, escribir registro completo; fallo → reintento único y degradado. Salida: registro o marca degradada. Val: VAL-06. Err: ERR-02.
...
### Objetivo
Persistir `capture_batch` en `"Ofertas Totales"` conservando información original, trazabilidad e `id_externo_url` crudo cuando exista, y liberar el lote. No normaliza ni verifica identidad más allá de `id_externo_url`. La deduplicación avanzada en dos capas pertenece al Módulo 2.

### NOTA — Desvío aprobado Sub-fase 4.4
- Se introdujo deduplicación por `id_externo_url` vía `upsert_oferta`.
- Si la fila existe: solo se refresca `timestamp_ultima_verificacion` y se conserva el `id`.
...
- MVP: catálogos `empresas`/`ubicaciones` no resueltos:
  ...
  - `source_id` guardado en `fuente_id` sin constraint FK.
...
Bajo NOTA aprobada: persistencia por oferta mediante upsert por `id_externo_url`.

Fallo de escritura: reintento único; si persiste, evento crítico y aborto. Tras persistir o no-op, libera lote.

### Entradas
Contexto: `capture_batch` con información original, `id_externo_url` best-effort, metadatos `run_id`, `source_id`, `session_id`, `set_indice`, referencia; conexión a `"Ofertas Totales"`.
...
- **RN-02** (con NOTA): upsert por `id_externo_url`; existente refresca `timestamp_ultima_verificacion` y conserva `id`; nuevo inserta. Deduplicación dos capas del Módulo 2 no se ejecuta aquí.
- **RN-03**: cada fila conserva información original íntegra, `id_externo_url` nullable, `empresa_nombre`/`ubicacion_nombre` crudos, `empresa_id`/`ubicacion_id` NULL en MVP, y trazabilidad: `run_id`, `source_id`, `session_id`, `set_indice`, timestamps de captura y última verificación.
...
- **VAL-02**: cada oferta tiene información original y metadatos; `id_externo_url` nullable.
...
| Código | Error / excepción | Detección | Registro | Acción | Estado |
|---|---|---:|---|---|---|
| ERR-01 | `capture_batch` ausente o corrupto | 1 | Evento crítico con `run_id`, `source_id`, `session_id`, `set_indice` | Aborto | `error` |
| ERR-02 | Fallo de escritura/transacción tras reintento único | 3 | Evento crítico con `run_id`, `source_id`, `session_id`, `set_indice` | Aborto | `error` |

### Escenarios límite
- Lote vacío por límite o fallo → no-op.
- Ofertas sin `id_externo_url` → campo nulo; Módulo 2 resolverá por capa difusa.
...
- **Sucesor de aborto**: `"Finalizar Proceso"`.
- **Contrato entregado**: `"Ofertas Totales"` como almacén crudo con trazabilidad e `id_externo_url`; lote liberado.
- **Impactos aprobados**:
  - decisión siguiente no depende de este nodo;
  - Módulo 2 aplicará deduplicación dos capas: estricta por `source_id` + `id_externo_url` normalizado; difusa por título+empresa+ubicación.

### Notas de implementación
- Base 1.0: insertar filas nuevas, transacción única, rollback completo.
- NOTA 4.4 aprobada: upsert por `id_externo_url` y persistencia por oferta.
...
3. **Persistir**. Entrada: lote. Proceso: upsert por oferta según NOTA: existente refresca timestamp; nuevo inserta con información original, crudos y trazabilidad. Fallo → reintento único; si persiste, aborto. Salida: lote persistido. Val: VAL-02, VAL-03. Err: ERR-02.
...
### Entradas
Contexto: `estado_captura`, `estado_paginacion`, políticas efectivas, `run_id`, `source_id`, `session_id`, `set_indice`.
...
### Descripción funcional
Lee iterador de sets, `search_result` del set corriente, `estado_captura` si `set_indice` coincide, y `limite_alcanzado`. Valida consistencia. Clasifica si la fuente está comprometida por fallo Grupo A. Evalúa:
```text
...
### Entradas
Contexto: iterador de sets, `search_result`, `estado_captura`, `limite_alcanzado`, `run_id`, `source_id`, `session_id`.
...
- **RN-03**: clasificación Grupo A / Grupo B propiedad exclusiva de este nodo:
  - **Grupo A — comprometen la fuente**: `sesion_expirada`, `bloqueo_plataforma`, `fuente_inalcanzable`, `timeout_consulta`, `timeout_captura`, `error_interno_consulta`, `error_interno_captura`.
  - **Grupo B — propios del set**: `filtros_no_aplicables`, `respuesta_invalida`.
- **RN-04**: resultado del set corriente se lee de `search_result` si búsqueda falló; de `estado_captura` solo si `estado_captura.set_indice == set_indice` corriente.
- **RN-05**: validar consistencia: iterador coherente con `source_id`; `search_result.set_indice` == set corriente.
...
### Validaciones
- **VAL-01**: insumos presentes.
- **VAL-02**: iterador coherente; `set_indice` alineado.
...
- **Aborto**: insumos ausentes/corruptos o iterador/`set_indice` incoherente.

### Errores

| Código | Error / excepción | Detección | Registro | Acción | Estado |
|---|---|---:|---|---|---|
| ERR-01 | Insumos ausentes/corruptos | 1 | Evento crítico con `run_id`, `source_id`, `session_id` | Aborto | `error` |
| ERR-02 | Iterador incoherente o `set_indice` desalineado | 2 | Evento crítico con `run_id`, `source_id`, `session_id` | Aborto | `error` |
...
2. **Validar consistencia**. Entrada: insumos. Proceso: iterador con `source_id`; `set_indice` corriente. Salida: validados. Val: VAL-02. Err: ERR-02.
...
|---|---|---|
| Contenido del evento de terminación | A. Minimal: `run_id`, timestamp, estado, motivo, `fuentes_procesadas` si disponible. B. Resumen completo. | **A**: el detalle ya existe en eventos por nodo; duplicar resúmenes acopla y ensucia. |
| Liberación del bloqueo | A. Incondicional. B. Condicional a propiedad: lock owner == `run_id`. | **B**: en ruta `concurrencia` esta corrida no posee el bloqueo; liberarlo rompería la corrida activa. |
...
2. **Registrar evento de terminación**. Entrada: estado, motivo, `run_id`, `fuentes_procesadas` si disponible. Proceso: escribir `{run_id, timestamp, estado, tipo, motivo, fuentes_procesadas}`; fallo → reintento único; si persiste, registro crítico local y continuar. Salida: evento persistido o registro local. Excepción: fallo tras reintento no aborta.
3. **Liberar bloqueo si corresponde**. Entrada: estado de bloqueo, `run_id`. Proceso: si bloqueo existe y propietario == `run_id`, liberar; fallo → reintento único; si persiste, registro local y continuar; si no propietario, sin acción. Salida: bloqueo liberado o sin acción. Excepción: fallo tras reintento no aborta.
