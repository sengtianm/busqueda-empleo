...
- Data rule (decision D31): no empty fields — persisted values that are not applicable/available use `N/A`/`N/R`, never `''`/NULL (exceptions: `ofertas.id_externo` to protect dedup, `ofertas.fecha_ultima_verificacion` by user instruction). `contar_distintos` treats `N/A` as empty.
...
- `pytest tests/` — test suite (currently 329 passing)
